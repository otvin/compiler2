...
There are 60 total errors in the ISO standard.  Compiler2 handles 16 of them, and has code to handle 2 others (but that code is not invoked yet).  The remaining errors apply to not-yet-implemented language features or are not detected.
...
|D.48|6.7.3|It is an error if the result of an activation of a function is undefined upon completion of the algorithm of the activation| Detected at runtime.  It is a compile-time error if a function does not contain at least one assignment to the return value. |
