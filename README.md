...
There are 60 total errors in the ISO standard.  Compiler2 handles 17 of them, and has code to handle 2 others (but that code is not invoked yet).  The remaining errors apply to not-yet-implemented language features or are not detected.
...
|D.32|6.6.6.2| Sqr(x) computes the square of x.  It is an error if such a value does not exist.                                                                                                                                                    | Detected at runtime. Reported as 'Overflow error,' not a reference to D.32 specifically.                                                   |
...
|D.48|6.7.3| It is an error if the result of an activation of a function is undefined upon completion of the algorithm of the activation                                                                                                         | Detected at runtime.  Additionally, it is a compile-time error if a function does not contain at least one assignment to the return value. |
